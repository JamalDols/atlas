...
function get_rss_feed_as_html($feed_url, $max_item_cnt = 10, $show_date = true, $show_description = true, $max_words = 0, $cache_timeout = 7200, $cache_prefix = "/tmp/rss2html-")
{
    ...
    for ($x=0;$x<$max_item_cnt;$x++) {
        ...
        $link = $feed[$x]['link'];
        $result .= '<div class="col-md-6 col-lg-3 card--news__home">';
        $result .= '<div class="item-news">';
        $result .= '<a class="permalink"  target="_blank"  href="'.$link.'"></a>';
        ...
        if ($show_description) {
            ...
            if ($has_image == 1) {
                $description = '<div class="image--container"><div class="image"style="background-image:url('. $image['src'] .')"></div></div>';
                
                

            }
            $result .= $description;
            $result .= '<div class="box-color">';
            $result .= ' <span class="date">'.$date.'</span>';
            $result .= ' <span class="text">'.$title.'</span></div>';
